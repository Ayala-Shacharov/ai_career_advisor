...
import { buildQuestionPrompt, buildProfessionPrompt, } from '../utils/prompt.util.js';
...
const parseJsonObject = (rawContent) => {
...
const validateQuestions = (payload) => {
    if (!payload ||
...
const validateProfession = (payload) => {
    if (!payload ||
        typeof payload !== 'object' ||
...
export class AIService {
    async callModel(systemPrompt, userText, temperature) {
    ...
    async matchProfession(text, answers) {
        try {
